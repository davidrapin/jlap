...
{
    private AtomicBoolean handlingChunks = new AtomicBoolean(false);

    @Override
    public void messageReceived(ChannelHandlerContext ctx, HttpRequest msg) throws Exception
    {
        if (handlingChunks.get()) {
            processChunk(ctx, msg);
        } else {
            processRequest(ctx, msg);
        }
    }

    private void processChunk(ChannelHandlerContext ctx, HttpRequest msg) {
        msg.
